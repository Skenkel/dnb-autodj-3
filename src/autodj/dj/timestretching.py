...
def time_stretch_sola(audio, f, sample_rate = 44100, wsola = False):
	...
	frame_len_1 = 4096 if wsola else 1024			# Length of a fragment, including overlap at one side; about 100 ms; shouldn't be longer because length of a 16th note is about this period at 175 BPM: otherwise it won't be possible to copy without doubling transients
	...
	output = np.zeros(num_samples_out + int(frame_len_1))		# f: stretch factor of audio!; prealloc
	...
	for out_ptr in range(0, int(num_frames_out * frame_len_1), int(frame_len_1)):
		...
		frame1_overlap = audio[int(in_ptr + frame_len_0) : int(in_ptr + frame_len_1 + 1)]  # +1 to prevent click noises due to occasional zero samples inserted in output
